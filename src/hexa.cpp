...
void hexa_get_27tree(hexa_tree_t *mesh, int nx, int ny, int nz, int step,
                     int level) {
  // int x,y,z;
...
void hexa_refinement_layer(hexa_tree_t *mesh, int nz, int coarse_step,
                           int internal_step, int level) {
...
void hexa_uniform_layer(hexa_tree_t *mesh, int nz, int coarse_step,
                        int internal_step, int level, bool nz_ext) {
  for (int ny = mesh->y_start; ny < mesh->y_end; ny += coarse_step) {
    for (int nx = mesh->x_start; nx < mesh->x_end; nx += coarse_step) {
      ...
      if (ny == mesh->y_start || !((ny + coarse_step) < mesh->y_end) ||
          nx == mesh->x_start || !((nx + coarse_step) < mesh->x_end) || nz_ext)
        elem->boundary = true;
...
void hexa_transient_layer(hexa_tree_t *mesh, int nz, int coarse_step,
                          int internal_step, int level) {
  for (int ny = mesh->y_start; ny < mesh->y_end; ny += coarse_step) {
    for (int nx = mesh->x_start; nx < mesh->x_end; nx += coarse_step) {
      int ext = 0;
      if (ny == mesh->y_start)
        ext = 1;
      if (!((ny + coarse_step) < mesh->y_end))
        ext = 2;
      if (nx == mesh->x_start)
        ext = 3;
      if (!((nx + coarse_step) < mesh->x_end))
        ext = 4;
      if (ny == mesh->y_start && nx == mesh->x_start)
        ext = 5;
      if (ny == mesh->y_start && !((nx + coarse_step) < mesh->x_end))
        ext = 6;
      if (!((nx + coarse_step) < mesh->x_end) &&
          !((ny + coarse_step) < mesh->y_end))
        ext = 7;
      if (!((ny + coarse_step) < mesh->y_end) && nx == mesh->x_start)
        ext = 8;
  ...
  while ((nz + internal_step) <= mesh->ncellz) {
    if (ccount < (int)nelZ.size() && (nlayer + 1) == nelZ[ccount]) {
      ...
      */
      ...
      bool nz_ext = false;
      if (nz == 0 || !((nz + 2 * internal_step) <= mesh->ncellz))
        nz_ext = true;
...
void hexa_tree_destroy(hexa_tree_t *mesh) {
  ...
  if (mesh->input.interfaceNumber != 0) {
    for (size_t k = 0; k < mesh->gdata_vec.size(); k++) {
      if (mesh->gdata_vec[k].bbt != NULL)
        gts_bb_tree_destroy(mesh->gdata_vec[k].bbt, TRUE);
    }
  }
}
