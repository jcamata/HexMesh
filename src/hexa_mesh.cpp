...
void hexaEdgesOld(hexa_tree_t* mesh,sc_hash_array_t* indep_nodes,sc_hash_array_t* shared_nodes,int64_t *local,int64_t *global,sc_hash_array_t* indep_edges,sc_hash_array_t* shared_edges)
{

	bool clamped = true;
	int num_indep_edges = 0;
	//insert all edges in the hash_array
	for(int iel = 0; iel < mesh->elements.elem_count; iel++)
	{
		octant_t *elem  = (octant_t*) sc_array_index(&mesh->elements, iel);
		...
		{
			size_t position;
			octant_edge_t* edge = &elem->edge[iedge];
			int Edge2GNode[2];
			int node1 = elem->nodes[EdgeVerticesMap[iedge][0]].id;
			int node2 = elem->nodes[EdgeVerticesMap[iedge][1]].id;
			edge->coord[0] = node1 <= node2 ? node1 : node2;
			edge->coord[1] = node1 >= node2 ? node1 : node2;
			edge->ref = false;
			edge->id = cantor_function_id(edge->coord[0],edge->coord[1]);
			elem->edge[iedge].id =  edge->id;
			octant_edge_t *r = (octant_edge_t*) sc_hash_array_insert_unique (indep_edges, edge, &position);
			if(r != NULL)
			{
				r->coord[0] = edge->coord[0];
				r->coord[1] = edge->coord[1];
				r->ref = edge->ref;
				r->id    = edge->id;
				//edge->id = edge->id;
				assert(position == num_indep_edges);
				++num_indep_edges;
			} else
			{
				//edge->id = position;
	...
	for (int iel = 0; iel < mesh->elements.elem_count; ++iel)
	{
		...
		for (int edge = 0; edge < 12; ++edge) {
			...
			size_t position0;
			size_t position1;
			bool ln0 =  sc_hash_array_lookup(shared_nodes, node0, &position0);
			bool ln1 =  sc_hash_array_lookup(shared_nodes, node1, &position1);
	...
	for(int i = 0; i < mesh->edges.elem_count; ++i)
	{
		octant_edge_t* e = (octant_edge_t*) sc_array_index(&mesh->edges,i);
		fprintf(mesh->fdbg, "(%d): %d %d\n", e->id, e->coord[0], e->coord[1]);
	}
#endif
	...
	for(int iedge = 0; iedge < mesh->shared_edges.elem_count; ++iedge){
		...
		for(int j = 0; j < se->listSz; j++)
		{
			...
			else if (se->rankList[j] > mesh->mpi_rank)
			{
				...
				mesh->global_edge_id[se->id] = -1;
			}
		}
		if(mesh->global_edge_id[se->id] == -1) not_my_edges++;
		if(mesh->global_edge_id[se->id] == -3) my_own_edges++;
	}
	...
	int offset = 0;
	MPI_Scan(&local[2], &offset, 3, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

	offset = offset-local[2];
	int count = offset;
	//for(int i=0; i < mesh->edges.elem_count; ++i) if(mesh->global_edge_id[i] != -1) mesh->global_edge_id[i] = count++;
	...
	fprintf(mesh->fdbg, "Edge (local) (proc) (global): \n");
	for(int i = 0; i < mesh->edges.elem_count; ++i)
	{
		octant_edge_t* n = (octant_edge_t*) sc_array_index(&mesh->edges,i);
		fprintf(mesh->fdbg, "(%d) (%d) (%ld): (%d %d) (%d %d)\n", n->id, mesh->part_nodes[n->coord[0]], mesh->global_edge_id[i],
				n->coord[0], n->coord[1],mesh->global_id[n->coord[0]],mesh->global_id[n->coord[1]]);
	}
#endif
