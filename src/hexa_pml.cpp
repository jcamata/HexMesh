		...
		if (face)
		{
			if (elem->surf[0].ext)
			{
				for (int n_l = 0; n_l < layers_x; ++n_l)
				{
					...
					pml_e->nodes[2].x = elem->nodes[aux[2]].x - 12 * (n_l + 0);
					pml_e->nodes[3].x = elem->nodes[aux[3]].x - 12 * (n_l + 1);
					pml_e->nodes[4].x = elem->nodes[aux[0]].x - 12 * (n_l + 1);
					pml_e->nodes[5].x = elem->nodes[aux[1]].x - 12 * (n_l + 0);
					pml_e->nodes[6].x = elem->nodes[aux[2]].x - 12 * (n_l + 0);
					pml_e->nodes[7].x = elem->nodes[aux[3]].x - 12 * (n_l + 1);
			...
			if (elem->edge[5].ref)
			{
				for (int nx = 0; nx < layers_x; nx++)
				{
					for (int ny = 0; ny < layers_y; ny++)
					{
						...
						y[2] = coords[3 * node0 + 1] - ny * Y_pml / layers_y;
						y[3] = coords[3 * node0 + 1] - ny * Y_pml / layers_y;
						y[0] = coords[3 * node0 + 1] - (ny + 1) * Y_pml / layers_y;
						y[1] = coords[3 * node0 + 1] - (ny + 1) * Y_pml / layers_y;
						y[6] = coords[3 * node1 + 1] - ny * Y_pml / layers_y;
						y[7] = coords[3 * node1 + 1] - ny * Y_pml / layers_y;
						y[4] = coords[3 * node1 + 1] - (ny + 1) * Y_pml / layers_y;
						y[5] = coords[3 * node1 + 1] - (ny + 1) * Y_pml / layers_y;

						pml_e->nodes[2].y = elem->nodes[aux[0]].y - 12 * (ny + 0);
						pml_e->nodes[3].y = elem->nodes[aux[0]].y - 12 * (ny + 0);
						pml_e->nodes[0].y = elem->nodes[aux[0]].y - 12 * (ny + 1);
						pml_e->nodes[1].y = elem->nodes[aux[0]].y - 12 * (ny + 1);
						pml_e->nodes[6].y = elem->nodes[aux[1]].y - 12 * (ny + 0);
						pml_e->nodes[7].y = elem->nodes[aux[1]].y - 12 * (ny + 0);
						pml_e->nodes[4].y = elem->nodes[aux[1]].y - 12 * (ny + 1);
						pml_e->nodes[5].y = elem->nodes[aux[1]].y - 12 * (ny + 1);
			...
			if (elem->edge[7].ref)
			{
				for (int nx = 0; nx < layers_x; nx++)
				{
					for (int ny = 0; ny < layers_y; ny++)
					{
						...
						x[1] = coords[3 * node0 + 0] - nx * X_pml / layers_x;
						x[0] = coords[3 * node0 + 0] - (nx + 1) * X_pml / layers_x;
						x[3] = coords[3 * node0 + 0] - (nx + 1) * X_pml / layers_x;
						x[2] = coords[3 * node0 + 0] - nx * X_pml / layers_x;
						x[5] = coords[3 * node1 + 0] - nx * X_pml / layers_x;
						x[4] = coords[3 * node1 + 0] - (nx + 1) * X_pml / layers_x;
						x[7] = coords[3 * node1 + 0] - (nx + 1) * X_pml / layers_x;
						x[6] = coords[3 * node1 + 0] - nx * X_pml / layers_x;

						pml_e->nodes[1].x = elem->nodes[aux[0]].x - 12 * (nx + 0);
						pml_e->nodes[0].x = elem->nodes[aux[0]].x - 12 * (nx + 1);
						pml_e->nodes[3].x = elem->nodes[aux[0]].x - 12 * (nx + 1);
						pml_e->nodes[2].x = elem->nodes[aux[0]].x - 12 * (nx + 0);
						pml_e->nodes[5].x = elem->nodes[aux[1]].x - 12 * (nx + 0);
						pml_e->nodes[4].x = elem->nodes[aux[1]].x - 12 * (nx + 1);
						pml_e->nodes[7].x = elem->nodes[aux[1]].x - 12 * (nx + 1);
						pml_e->nodes[6].x = elem->nodes[aux[1]].x - 12 * (nx + 0);
			...
			if (elem->edge[9].ref)
			{
				...
				for (int nz = 0; nz < layers_z; nz++)
				{
					for (int nx = 0; nx < layers_x; nx++)
					{
						...
						z[6] = coords[3 * node1 + 2] - (nz + 1) * Z_pml / layers_z;
						z[7] = coords[3 * node1 + 2] - (nz + 1) * Z_pml / layers_z;
						z[0] = coords[3 * node0 + 2] - (nz)*Z_pml / layers_z;
						z[1] = coords[3 * node0 + 2] - (nz)*Z_pml / layers_z;
						z[2] = coords[3 * node1 + 2] - (nz)*Z_pml / layers_z;
						z[3] = coords[3 * node0 + 2] - (nz)*Z_pml / layers_z;

						pml_e->nodes[4].z = elem->nodes[aux[0]].z + 12 * (nz + 1);
						pml_e->nodes[5].z = elem->nodes[aux[0]].z + 12 * (nz + 1);
						pml_e->nodes[6].z = elem->nodes[aux[1]].z + 12 * (nz + 1);
						pml_e->nodes[7].z = elem->nodes[aux[1]].z + 12 * (nz + 1);
						pml_e->nodes[0].z = elem->nodes[aux[0]].z + 12 * (nz + 0);
						pml_e->nodes[1].z = elem->nodes[aux[0]].z + 12 * (nz + 0);
						pml_e->nodes[2].z = elem->nodes[aux[1]].z + 12 * (nz + 0);
						pml_e->nodes[3].z = elem->nodes[aux[1]].z + 12 * (nz + 0);
			...
			if (elem->edge[10].ref)
			{
				for (int nz = 0; nz < layers_z; nz++)
				{
					for (int ny = 0; ny < layers_y; ny++)
					{
						...
						pml_e->nodes[4].z = elem->nodes[aux[0]].z + 12 * (nz + 1);
						pml_e->nodes[5].z = elem->nodes[aux[1]].z + 12 * (nz + 1);
						pml_e->nodes[6].z = elem->nodes[aux[1]].z + 12 * (nz + 1);
						pml_e->nodes[7].z = elem->nodes[aux[0]].z + 12 * (nz + 1);
						pml_e->nodes[0].z = elem->nodes[aux[0]].z + 12 * (nz + 0);
						pml_e->nodes[1].z = elem->nodes[aux[1]].z + 12 * (nz + 0);
						pml_e->nodes[2].z = elem->nodes[aux[1]].z + 12 * (nz + 0);
						pml_e->nodes[3].z = elem->nodes[aux[0]].z + 12 * (nz + 0);
				...
				for (int nx = 0; nx < layers_x; nx++)
				{
					for (int ny = 0; ny < layers_y; ny++)
					{
						for (int nz = 0; nz < layers_z; nz++)
						{
							...
							pml_e->nodes[4].z = elem->nodes[aux].z + 12 * (nz + 1);
							pml_e->nodes[5].z = elem->nodes[aux].z + 12 * (nz + 1);
							pml_e->nodes[6].z = elem->nodes[aux].z + 12 * (nz + 1);
							pml_e->nodes[7].z = elem->nodes[aux].z + 12 * (nz + 1);
							pml_e->nodes[0].z = elem->nodes[aux].z + 12 * (nz + 0);
							pml_e->nodes[1].z = elem->nodes[aux].z + 12 * (nz + 0);
							pml_e->nodes[2].z = elem->nodes[aux].z + 12 * (nz + 0);
							pml_e->nodes[3].z = elem->nodes[aux].z + 12 * (nz + 0);
